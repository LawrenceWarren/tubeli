...
parser = argparse.ArgumentParser(
    description='London Underground Live Departure Board, to run the program you will need to pass it all of the required paramters and you may wish to pass any optional paramters.'
)
parser.add_argument(
    "-t",
    "--TimeFormat",
...
parser.add_argument(
    ...
    "--ExcludeLines",
    default="",
    help="List any Lines you do not wish to view. Make sure to capitalise correctly and simply put a single space between each, for example 'Bakerloo Circle'; default is nothing, ie show every service.",
    nargs='*',
)
parser.add_argument(
    "-p",
    "--Direction",
    help="For stations which have inbound and outbound services, do you wish to view both directions or only one?; default is both directions",
...
parser.add_argument(
    "-w",
    "--WarningTime",
    help="How soon before the warning message will be displayed about a trains arrival in min; 0.2 by default. i.e when the train is due to arrive in 0.2min start warning of an arriving train.",
    type=check_positive,
    default=0.2,
)
parser.add_argument(
    ...
    dest='ShowIndex',
    action='store_false',
    help="Do you wish to see index position for each service due to arrive. By default yes.",
    default=True,
)
parser.add_argument(
    "--IncreasedAnimations",
    help="If you wish to show an additional animation message which shows 'This is a [Line Name] line train, to [destination]' turn it on with the following; by default this animation isn't shown as it will be the same for a lot of services.",
...
parser.add_argument(
    ...
    default=False,
    action='store_true',
    help="Do you want the warning 'STAND BACK TRAIN APPROACHING' message to flash; off by default.",
)
parser.add_argument(
    "--Display",
    default="ssd1322",
...
requiredNamed.add_argument(
    "-k",
    "--APIKey",
    help="Your Transport for London API Key, you can get your own at: https://api-portal.tfl.gov.uk/signup",
    type=str,
    required=True,
)
requiredNamed.add_argument(
    "-s",
    "--StationID",
    help="The London Underground Code for the specific station you wish to display.",
    type=str,
    required=True,
)
...
class LiveTime(object):
    ...
    def __init__(self, Data):
        ...
        self.ID = str(Data['id'])
        self.Via = "This is a %s line train, to %s" % (
            str(Data['lineName']),
            str(Data['destinationName'] if 'destinationName' in Data else str(Data['towards'])),
        )
    ...
    def TimePassedStatic(self):
        return ("min" in self.DisplayTime) and (
            datetime.now() - self.LastStaticUpdate
        ).total_seconds() > Args.StaticUpdateLimit
    ...
    def GetData():
        ...
        url = "https://api.tfl.gov.uk/StopPoint/%s/Arrivals?app_id=%s&app_key=%s" % (
            Args.StationID,
            Args.APIKey,
            Args.APIKey,
        )
            ...
            with urlopen(req) as conn:
                ...
                for service in tempServices:
                    ...
                    if str(service['lineName']) not in Args.ExcludeLines:
                        if Args.Direction == 'both' or (
                            "direction" in service and Args.Direction == str(service["direction"])
                        ):
...
class ScrollTime:
    ...
    def __init__(
        self, image_composition, service, previous_service, scroll_delay, synchroniser, device, position, controller
    ):
        ...
        self.IStaticOld = ComposableImage(
            StaticTextImage(device, service, previous_service).image, position=(0, (16 * position))
        )
        ...
        self.device = device
        self.partner = None
        self.TrainApproaching = ComposableImage(
            TextImage(device, "* STAND BACK TRAIN APPROACHING *").image, position=(0, 16 * self.position)
        )
        self.Blank = ComposableImage(
            TextImage(device, "                                ").image, position=(0, 16 * position + 16)
        )
    ...
    def generateCard(self, service):
        ...
        self.IDestination = ComposableImage(
            IDestinationTemp.image.crop((0, 0, IDestinationTemp.width + 10, 16)), position=(0, 16 * self.position)
        )
        self.IDisplayTime = ComposableImage(
            displayTimeTemp.image, position=(device.width - displayTimeTemp.width, 16 * self.position)
        )
    ...
    def changeCard(self, newService, device):
        ...
        self.IStaticOld = ComposableImage(
            StaticTextImage(device, newService, self.CurrentService).image, position=(0, (16 * self.position))
        )
    ...
    def tick(self):
        ...
        if self.CurrentService.TimePassedStatic() and (
            self.state == self.SCROLL_DECIDER
            or self.state == self.SCROLLING_WAIT
            or self.state == self.SCROLLING
...
class boardFixed:
    def __init__(self, image_composition, scroll_delay, device):
        ...
        self.NoServices = ComposableImage(
            NoServiceTemp.image,
            position=(
                int(device.width / 2 - NoServiceTemp.width / 2),
                int(device.height / 2 - NoServiceTemp.height / 2),
            ),
        )
    ...
    def setInitalCards(self):
        ...
        self.top = ScrollTime(
            image_composition,
            len(self.Services) >= 1 and self.Services[0] or LiveTimeStud(),
            LiveTimeStud(),
            self.scroll_delay,
            self.synchroniser,
            device,
        ...
        self.middel = ScrollTime(
            image_composition,
            len(self.Services) >= 2 and self.Services[1] or LiveTimeStud(),
            LiveTimeStud(),
            self.scroll_delay,
            self.synchroniser,
            device,
        ...
        self.bottom = ScrollTime(
            image_composition,
            len(self.Services) >= 3 and self.Services[2] or LiveTimeStud(),
            LiveTimeStud(),
            self.scroll_delay,
            self.synchroniser,
            device,
...
FontTime = ImageFont.truetype(
    "%s/resources/time.otf" % (os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))), 16
)
device.contrast(255)
energyMode = "normal"
...
def display():
    ...
    with canvas(device, background=image_composition()) as draw:
        ...
        draw.multiline_text(
            ((device.width - int(draw.textlength(msgTime, FontTime))) / 2, device.height - 16),
            msgTime,
            font=FontTime,
            align="center",
        )
...
def Splash():
    if Args.SplashScreen:
        with canvas(device) as draw:
            ...
            draw.multiline_text(
                (45, 35),
                "Version : 2.11.LU -  By Jonathan Foot",
                font=ImageFont.truetype(
